...
import { baseURL } from '../Signup/FacultySignup';
...
const ResetPassFac = () => {
 ...
 const handleUsernameChange = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const sendOTP = async () => {
    try {
      await axios.post(baseURL+'/sendemail', { email: username});
  ...
  const verifyOTP = async () => {
    try {
      ...
      navigate('/change-pass/faculty', { state: { username } });
    } catch (error) {
